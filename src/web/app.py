"""Single-page pipeline infographic for the Case 3 Media Behavior project.

Tells the data-pipeline story in one scroll:

    1. Raw data
    2. Cleaning
    3. Feature selection (ANOVA, top 5)
    4. Unsupervised (K-Means personas)
    5. Supervised (RandomForest + Logistic Regression coefficients)
    6. Predict — interactive form
...
.stage-card::before {
    ...
    inset: 0 auto 0 0;
    width: 4px;
    background: var(--stage-color, #22d3ee);
...
.stage-num {
    ...
    font-weight: 800;
    margin-right: 12px;
    box-shadow: 0 0 24px var(--stage-color, #22d3ee);
}
.stage-title {
    color: var(--text);
    font-size: 22px;
...
# ---------------------------------------------------------------------------
stage_open(3, "#34d399", "Feature Selection",
           "Rule · top 5 source variables by ANOVA F-score (deduplicated)")
if not feature_summary_df.empty:
    top5 = (
        feature_summary_df.sort_values("f_score", ascending=False)
        .drop_duplicates("source_var")
        .head(5)
    )
...
# ---------------------------------------------------------------------------
stage_open(4, "#f472b6", "Unsupervised — Customer Personas",
...
if metrics:
    ...
    if not coef_df.empty:
        ...
        for _, row in ranked.iterrows():
            ...
            pct = abs(coef) / max_abs * 50  # half-width = 50%
            if coef >= 0:
                bar = (
                    f"<div class='coef-bar-wrap'>"
                    f"<div class='coef-bar-pos' style='width:{pct:.1f}%'></div>"
                    f"</div>"
                )
            else:
                bar = (
                    f"<div class='coef-bar-wrap'>"
                    f"<div class='coef-bar-neg' style='width:{pct:.1f}%'></div>"
                    f"</div>"
                )
...
with st.form("predict_form", clear_on_submit=False):
    ...
    # Sliders for the numeric (Likert) features.
    ...
    model_choice = st.radio(
        "Model", [best_name, "LogisticRegression"], index=0, horizontal=True,
    )
    submitted = st.form_submit_button("Predict ✨", type="primary")

if submitted:
    feature_order = (
        [s["name"] for s in schema["numeric"]]
        + [s["name"] for s in schema["categorical"]]
    )
